...
\begin{frame}{Signal Loss Due to Source Resistance}
    ...
    \begin{tikzpicture}
        ...
        \node at (axis cs:100,8.5) {\small 80\% of $v_s$};
...
\begin{frame}{Arbitrary Time-Domain Signals}
    ...
    \begin{block}{Solution}
        Frequency spectrum representation!  (Next section)
    \end{block}
...
\begin{frame}{The Sinusoid:  Building Block of Signals}
    ...
    \column{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            % % Amplitude arrow
            % \draw[<->, thick, green! 60!black] (axis cs: 0.2,0) -- (axis cs:0.2,1);
            % \node[right] at (axis cs:0.2,0.5) {\small $V_a$};
...
%     \textbf{Problem 1}: A signal source has open-circuit voltage \SI{10}{mV} and short-circuit current \SI{10}{\micro A}. Find $R_s$.
    
%     \vspace{0.3cm}
    
%     \textbf{Problem 2}: For a Thevenin source with $v_s = \SI{10}{mV}$ and $R_s = \SI{1}{k\ohm}$, find $v_o$ for: 
%     \begin{itemize}
%         \item $R_L = \SI{100}{k\ohm}$
%         \item $R_L = \SI{10}{k\ohm}$
%         \item $R_L = \SI{1}{k\ohm}$
%         \item $R_L = \SI{100}{\ohm}$
%     \end{itemize}
...
%     \textbf{Problem 3}: What is the lowest $R_L$ for which $v_o \geq 0.8 v_s$?
    
%     \vspace{0.3cm}
    
%     \textbf{Problem 4}: A sine wave has RMS value \SI{120}{V}. Find its peak value.
    
%     \vspace{0.3cm}
    
%     \textbf{Problem 5}: Sketch the spectrum of $v(t) = 2\sin(\omega_0 t) + \sin(2\omega_0 t) + 0.5\sin(3\omega_0 t)$
