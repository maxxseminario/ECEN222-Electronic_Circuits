...
\large Lab 1 — ECEN 222: Electronic Circuits}
\author{
    % Maxx Seminario\\
...
\begin{itemize}
    \item Small-signal silicon diode (PN junction)
    \item Zener diode
    \item Resistors
\end{itemize}
...
In this portion of the experiment, you will measure the forward-bias current-voltage characteristics of a silicon diode. Begin by constructing the circuit shown in Figure \ref{fig:forward_circuit} on your breadboard using the small-signal silicon diode and the 1 k$\Omega$ resistor. Ensure that the diode is oriented correctly with the anode connected toward the positive supply terminal and the cathode toward ground. Connect one DMM across the diode terminals to measure the diode voltage $V_D$ directly.  To measure the current through the diode $I_D$, you have two options:  either connect the second DMM in series with the circuit (being careful to observe the current measurement limits of your meter), or measure the voltage across the resistor and calculate the current using Ohm's law ($I_D = V_R / R$). The second method is often preferred as it avoids potential issues with meter burden voltage. \\
...
Starting with the power supply set to 0 V, gradually increase the supply voltage $V_S$ in small increments.  For each supply voltage setting, record both the voltage across the diode $V_D$ and the current through the diode $I_D$. Pay particular attention to taking measurements at diode voltages around from 0 V to 0.8 V. You may need to adjust the supply voltage carefully to achieve specific diode voltages. Take special care in the region between 0.5 V and 0.8 V where the current changes rapidly. You should observe that very little current flows until the diode voltage reaches approximately 0.5 to 0.6 V, after which the current increases exponentially. \\

Exercise caution not to exceed a diode current of approximately 20 mA for silicon diodes, as excessive current can cause damage.  If your diode is heating up noticeably, reduce the current immediately. Once you have completed measurements for the small-signal diode, repeat the entire procedure using the silicon rectifier diode.  You should observe similar behavior but may notice slight differences in the forward voltage drop characteristics between the two diode types. \\

In your lab report, you will plot the I-V characteristics for the diode.  Describe what you observe about the shape of the curve and how it relates to the Shockley diode equation.  Explain why very little current flows below a certain voltage and why the current increases so rapidly once this threshold is exceeded.  \\

\subsection{Part 2: Small-Signal Resistance Analysis}

Using the data you collected in Part 1, you will now determine the small-signal or small signal resistance of the diode at different operating points. Select two distinct operating points in the forward-bias region where the diode is conducting appreciable current.  Suggested operating points are around $V_{D1} \approx 0.65$ V and $V_{D2} \approx 0.75$ V, though the exact values will depend on your measured data. \\
...
where $\Delta V_D$ and $\Delta I_D$ represent small changes in voltage and current around the operating point. To obtain accurate results, select two measurement points that are close to (but on either side of) your chosen operating point, and calculate the slope between these two points. The smaller the interval you use (while still maintaining measurement accuracy), the better your approximation of the true small signal resistance. \\

After calculating the experimental small signal resistance values, compare them with the theoretical predictions from Equation \ref{eq:dynamic_r}. For this comparison, you will need to know (or assume) the ideality factor $n$ and use the measured DC current at each operating point.  Calculate the percentage error between your measured and theoretical values.  In your lab report, discuss what you observe about how the small signal resistance changes with operating current.  Explain physically why the small signal resistance decreases as the forward current increases.  Also discuss possible sources of error in your measurements and how they might affect the accuracy of your small signal resistance calculations. \\

\subsection{Part 3: Reverse-Bias Characteristics of Silicon Diodes}

In this section, you will investigate the behavior of a silicon diode under reverse-bias conditions. Carefully remove the diode from your circuit and reverse its orientation so that the cathode is now connected toward the positive supply terminal and the anode toward ground as shown in Figure \ref{fig:reverse_circuit}.  This reverse-biases the diode.  To limit the current in case of accidental breakdown, change the resistor value to 470 $\Omega$. \\
...
Starting from 0 V, gradually increase the supply voltage, which now applies an increasing reverse voltage across the diode.  Take measurements of the reverse voltage $V_D$ (which will be negative) and the reverse current $I_D$ at increments of approximately 1 V, up to the maximum rated reverse voltage of your diode. Consult the diode datasheet or ask your instructor for the maximum reverse voltage rating.  Do not exceed this voltage, as doing so may cause unintended breakdown and permanent damage to regular junction diodes. \\

You should observe that the reverse current is extremely small, likely in the range of nanoamperes or microamperes, and may be difficult to measure accurately with standard multimeters. The current should remain relatively constant as you increase the reverse voltage, consistent with the saturation current $I_S$ predicted by the Shockley equation.  If you observe rapidly increasing current, you have likely exceeded the breakdown voltage and should immediately reduce the voltage. \\
...
From the measurements you made in Part 4, you will now determine the small signal resistance of the Zener diode in its breakdown region. Select two measurement points where the Zener is clearly in breakdown (i.e., where the supply voltage significantly exceeds the Zener voltage). Choose points with substantially different currents—for example, one point where $V_S \approx 7$ V and another where $V_S \approx 10$ V—to get a meaningful calculation. \\
...
where $\Delta V_Z$ is the change in Zener voltage and $\Delta I_Z$ is the change in Zener current between your two selected points. A small value of $r_z$ (typically less than 50 $\Omega$) indicates good voltage regulation, as it means the voltage changes very little with current.  A larger $r_z$ indicates poorer regulation. \\
