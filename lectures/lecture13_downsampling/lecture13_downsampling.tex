...
\begin{frame}{Downsampling: Motivation and Key Ideas}
\small
...
\begin{itemize}
  ...
  \item Enable multirate algorithms, filter banks, and pyramids.
  \item Coarser-time analysis or stage-wise processing at lower rates.
\end{itemize}
...
\begin{frame}{Downsampling: When is it Alias-Free?}
...
\begin{tikzpicture}[scale=0.78]
  ...
  \addplot[red, very thick] coordinates {(-3.4,0) (-3.1416,0) (-1.0472,1) (1.0472,1) (3.1416,0) (3.4,0)};

  \nextgroupplot[
    title={$\tilde{X}(e^{j\omega})$},
    xmin=-3.4, xmax=3.4,
    xtick={-1.0472,0,1.0472},
    xticklabels={$-\pi/3$,$0$,$\pi/3$}, % FIXED: added '=' after 'xticklabels'
    xlabel={$\omega$}
  ]
  \addplot[blue, very thick] coordinates {(-3.4,0) (-1.0472,0) (0,1) (1.0472,0) (3.4,0)};

  \nextgroupplot[
    title={$\tilde{X}_d(e^{j\omega}),\,M=3$},
    xmin=-3.4, xmax=3.4,
    xtick={-3.1416,0,3.1416},
    xticklabels={$-\pi$,$0$,$\pi$},
    xlabel={$\omega$}
  ]
  \addplot[blue, very thick] coordinates {(-3.4,0) (-3.1416,0) (0,0.33) (3.1416,0) (3.4,0)};
...
Notes:
\begin{itemize}
    \item $\tilde{x}_d[n] \neq x[nM]$; it is from the filtered version.
    \item Trade-off: reduced bandwidth for alias-free rate reduction.
\end{itemize}
\end{frame}
...
\textbf{Applications}:
\begin{itemize}
    \item Audio/video sample-rate conversion
    \item Multirate filter banks
    \item Computational load reduction
    \item Data compression pipelines
\end{itemize}
\end{frame}
